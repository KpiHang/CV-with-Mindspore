...
class Expand:
    ...
    def __call__(self, img, boxes, labels, mask):
        ...
        expand_mask = np.zeros((mask_count, int(mask_h * ratio),
                                int(mask_w * ratio))).astype(mask.dtype)
...
def rescale_column(img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    ...
    img_data, scale_factor = \
        rescale_with_tuple(img, (config.img_width, config.img_height))
    if img_data.shape[0] > config.img_height:
        img_data, scale_factor2 = \
            rescale_with_tuple(img_data, (config.img_height,
                                          config.img_height))
    ...
    pad_img_data = np.zeros((config.img_height,
                             config.img_width, 3)).astype(img_data.dtype)
    pad_img_data[0:img_data.shape[0], 0:img_data.shape[1], :] = img_data

    mask_count, mask_h, mask_w = gt_mask_data.shape
    pad_mask = np.zeros((mask_count, config.img_height,
                         config.img_width)).astype(gt_mask_data.dtype)
...
def resize_column(img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    ...
    img_data = cv2.resize(img_data, (config.img_width, config.img_height),
    ...
    gt_bboxes = gt_bboxes * scale_factor
    gt_bboxes[:, 0::2] = np.clip(gt_bboxes[:, 0::2],
                                 0, img_shape[1] - 1)
    gt_bboxes[:, 1::2] = np.clip(gt_bboxes[:, 1::2],
                                 0, img_shape[0] - 1)

    gt_mask_data = np.array([
        cv2.resize(mask, (config.img_width, config.img_height),
...
def impad_to_multiple_column(img, img_shape, gt_bboxes,
                             gt_label, gt_num, gt_mask):
    """impad operation for image"""
    img_data = cv2.copyMakeBorder(img,
                                  0, config.img_height - img.shape[0],
                                  0, config.img_width - img.shape[1],
                                  cv2.BORDER_CONSTANT,
                                  value=0)
...
def expand_column(img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    ...
    expand = Expand()
    img, gt_bboxes, gt_label, gt_mask = \
        expand(img, gt_bboxes, gt_label, gt_mask)

    return (img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask)


def pad_to_max(img, img_shape, gt_bboxes, gt_label,
               gt_num, gt_mask, instance_count):
    """pad the image by the max instance count."""
    pad_max_number = config.max_instance_count
    gt_box_new = np.pad(gt_bboxes,
                        ((0, pad_max_number - instance_count), (0, 0)),
                        mode="constant", constant_values=0)
    gt_label_new = np.pad(gt_label,
                          ((0, pad_max_number - instance_count)),
                          mode="constant", constant_values=-1)
    gt_iscrowd_new = np.pad(gt_num,
                            ((0, pad_max_number - instance_count)),
                            mode="constant", constant_values=1)
    gt_iscrowd_new_revert = ~(gt_iscrowd_new.astype(np.bool))

    return(img, img_shape, gt_box_new, gt_label_new,
           gt_iscrowd_new_revert, gt_mask)


def preprocess_fn(image, box, mask, mask_shape, is_training):
    """Preprocess function for dataset."""
    def _infer_data(image_bgr, image_shape, gt_box_new,
                    gt_label_new, gt_iscrowd_new_revert,
                    gt_mask_new, instance_count):
        image_shape = image_shape[:2]
        input_data = image_bgr, image_shape, gt_box_new, gt_label_new, \
            gt_iscrowd_new_revert, gt_mask_new
    ...
    def _data_aug(image, box, mask, mask_shape, is_training):
        ...
        if not is_training:
            return _infer_data(image_bgr, image_shape,
                               gt_box, gt_label, gt_iscrowd,
                               gt_mask, instance_count)

        flip = (np.random.rand() < config.flip_ratio)
        expand = (np.random.rand() < config.expand_ratio)

        input_data = image_bgr, image_shape, gt_box, \
            gt_label, gt_iscrowd, gt_mask
    ...
    for ind, img_id in enumerate(image_ids):
        ...
        if not is_training:
            ...
            if annos:
                ...
                instance_masks = \
                    np.stack(instance_masks, axis=0).astype(np.bool)
                masks[image_path] = np.array(instance_masks).tobytes()
                masks_shape[image_path] = \
                    np.array(instance_masks.shape, dtype=np.int32)
            else:
                print("no annotations for image ", file_name)
                image_anno_dict[image_path] = np.array([0, 0, 0, 0, 0, 1])
                masks[image_path] = \
                    np.zeros([1, image_height, image_width],
                             dtype=np.bool).tobytes()
                masks_shape[image_path] = \
                    np.array([1, image_height, image_width], dtype=np.int32)
...
def data_to_mindrecord_byte_image(dataset="coco",
    ...
    if dataset == "coco":
        image_files, image_anno_dict, masks, masks_shape = \
    ...
    for ind, image_name in enumerate(image_files):
        ...
        if (ind + 1) % 10 == 0:
            print("writing {}/{} into mindrecord".format(ind + 1,
                                                         image_files_num))
        writer.write_raw_data([row])
    writer.commit()


def create_coco_dataset(mindrecord_file, batch_size=1,
    ...
    compose_map_func = (lambda image, annotation, mask, mask_shape:
                        preprocess_fn(image, annotation, mask,
                                      mask_shape, is_training))
    ...
    if is_training:
        ...
        ds = ds.batch(batch_size,
                      drop_remainder=True,
                      pad_info={"mask": ([config.max_instance_count,
                                          None, None], 0)})
